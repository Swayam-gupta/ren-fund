...
def _panel_scorecard(ax, equity: pd.Series, trades: pd.DataFrame):
    ...
    y    = 0.88
    step = 0.055   # tighter rows to fit more metrics

    for k, v in metrics.items():
        # Format value cleanly
        if isinstance(v, float):
            if math.isnan(v):   v_str = "n/a"
            elif math.isinf(v): v_str = "∞"
            else:               v_str = str(v)
        else:
            v_str = str(v)

        # Colour logic
        is_num = isinstance(v, (int, float)) and not (isinstance(v, float) and (math.isnan(v) or math.isinf(v)))
        is_neg = is_num and v < 0
        is_pos = is_num and v > 0

        GREEN_KEYS = ("Return","CAGR","Ann.","Sharpe","Sortino","Factor","Win","Expectancy","Calmar")
        RED_KEYS   = ("Drawdown","Avg Loss")

        if any(rk in k for rk in RED_KEYS):
            col = RED
        elif any(gk in k for gk in GREEN_KEYS) and is_pos:
            col = GREEN
        elif is_neg:
            col = RED
        elif v_str == "∞":
        ...
        ax.text(0.04, y, k, ha="left", va="top",
                transform=ax.transAxes, color=GREY, fontsize=8.0)
        ax.text(0.97, y, v_str, ha="right", va="top",
                transform=ax.transAxes, color=col, fontsize=8.0, fontweight="bold")
        ax.plot([0.02, 0.98], [y - 0.006, y - 0.006],
                color=GRID, lw=0.35, transform=ax.transAxes)
        y -= step
